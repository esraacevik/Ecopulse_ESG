...
**Dosyalar**:
- `modeller/api/ml_routes.py`
- `modeller/models/energy_prediction/predictor.py`
...
**Durum**: Raporlar `output/` klasöründe ama liste yok  
...
**Dosyalar**:
- `backend/app/api/v1/report.py` (yeni endpoint'ler)
- `frontend/src/components/ReportHistory.tsx` (yeni component)
- `frontend/src/services/api.ts` (yeni API çağrıları)
- `frontend/src/app/page.tsx` (yeni tab/sayfa)
...
**Dosyalar**:
- `frontend/src/components/MLDashboard.tsx`
- `frontend/src/services/api.ts` (forecast endpoint)
...
```python
# report.py içinde
REPORTS_METADATA_FILE = Path("output/reports_metadata.json")
...
- Model checkpoint yolu: `modeller/checkpoints/weather_training/`
- Rapor output yolu: `output/`
