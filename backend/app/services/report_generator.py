...
class ESG_Report_Generator:
    ...
    def generate_report(self, results: List[EmissionResult], scope_summary: dict, filename: str = "esg_report.pdf", 
        ...
        if ml_results:
            story.append(PageBreak())
            story.append(Paragraph("4. Akilli Analiz Sonuclari", self.styles['CustomHeading']))
            
            # Benchmark sonuçları
            if ml_results.get('benchmark'):
                benchmark = ml_results['benchmark']
                story.append(Paragraph("<b>4.1 Sektor Karsilastirmasi</b>", self.styles['CustomNormal']))
                story.append(Spacer(1, 0.3*cm))
                
                if benchmark.get('success') and benchmark.get('metrics'):
                    metrics = benchmark['metrics']
                    benchmark_text = f"""
                    <b>Sektor:</b> {benchmark.get('sector', 'Belirtilmemis')}<br/>
                    <b>Sirket Yogunlugu:</b> {metrics.get('company_intensity', 0):.4f} kg CO2e/USD<br/>
                    <b>Sektor Ortalamasi:</b> {metrics.get('sector_intensity', 0):.4f} kg CO2e/USD<br/>
                    <b>Performans Orani:</b> {metrics.get('ratio', 0):.2f}x<br/>
                    <b>Sektor Ici Siralama:</b> En iyi %{100 - metrics.get('percentile', 50)}<br/>
            ...
            if ml_results.get('target'):
                ...
                story.append(Paragraph("<b>4.2 Net Zero Yol Haritasi</b>", self.styles['CustomNormal']))
                story.append(Spacer(1, 0.3*cm))
                ...
                if target.get('success') and target.get('summary'):
                    ...
                    <b>Hedef Yil:</b> {summary.get('target_year', 2030)}<br/>
                    <b>Hedef Emisyon:</b> {summary.get('target_emissions', 0):,.0f} ton CO2e<br/>
                    <b>Toplam Azaltim:</b> {summary.get('total_reduction', 'N/A')}<br/>
                    <b>SBTi Uyumlu:</b> {'Evet' if summary.get('sbti_aligned') else 'Hayir'}
                    """
                ...
                if target.get('milestones'):
                    story.append(Spacer(1, 0.3*cm))
                    milestone_data = [['Yil', 'Hedef (ton)', 'Azaltim']]
                ...
                # Investment
                if target.get('investment'):
                    inv = target['investment']
                    story.append(Spacer(1, 0.5*cm))
                    story.append(Paragraph("<b>Yatirim Tahmini:</b>", self.styles['CustomNormal']))
                    inv_text = f"""
                    Toplam Yatirim: {inv.get('total_investment', 'N/A')}<br/>
                    Yillik Tasarruf: {inv.get('estimated_annual_savings', 'N/A')}<br/>
                    Geri Odeme Suresi: {inv.get('payback_period', 'N/A')}
                    """
                    story.append(Paragraph(inv_text, self.styles['CustomNormal']))
                
                story.append(Spacer(1, 0.5*cm))

        # ==================== METODOLOJI ====================
        story.append(PageBreak())
        section_num = "5" if ml_results else "4"
        story.append(Paragraph(f"{section_num}. Metodoloji", self.styles['CustomHeading']))

        methodology = f"""
        <b>{section_num}.1 Hesaplama Standardi:</b> GHG Protocol Corporate Accounting and Reporting Standard

        <br/><br/>

        <b>{section_num}.2 Emission Factor Kaynagi:</b> Climatiq API (80+ global veri kaynagi)

        <br/><br/>

        <b>{section_num}.3 Hesaplama Formulu:</b><br/>
        CO2e = Aktivite Verisi x Emission Factor

        <br/><br/>

        <b>{section_num}.4 Scope Tanimlari:</b><br/>
        ...
        <b>{section_num}.5 Konsolidasyon Yaklasimi:</b> Operasyonel Kontrol

        <br/><br/>

        <b>{section_num}.6 Veri Kalitesi:</b> Birincil veri (olcum ve faturalar)
        """
